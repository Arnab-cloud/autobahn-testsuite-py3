...
from autobahn.websocket.http import BAD_REQUEST, HttpException
...
class WsPerfMasterProtocol(WebSocketServerProtocol):
    ...
    def toMicroSec(self, value):
        return ("%." + str(self.factory.digits) + "f") % round(
            float(value), self.factory.digits
        )
    ...
    def onConnect(self, connectionRequest):
        if "wsperf" in connectionRequest.protocols:
            ...
            raise HttpException(
                BAD_REQUEST[0], "You need to speak wsperf subprotocol with this server!"
            )
        ...
        if not binary:
            if msg is not None:
                try:
                    ...
                    self.protocolError(
                        "could not decode text message as JSON (%s)" % str(e)
                    )
            else:
...
def startServer(self, webport, debug=False):
    ...
    if webport:
        webdir = File(
            pkg_resources.resource_filename("autobahntestsuite", "web/wsperfmaster")
        )
        web = Site(webdir)
...
# class WsPerfMasterUiSession(ApplicationSession):
#     def __init__(self, config):
#         super().__init__(config)
#         self.service = config.extra["service"]
#         self.service.session = self

#     @register("com.example.run_case")
#     async def run_case(self, caseDef):
#         return self.service.runCase(caseDef)

#     @register("com.example.get_slaves")
#     async def get_slaves(self):
#         return self.service.getSlaves()

#     def onJoin(self, details):
#         print("Session ready")
#         self.service.sessions.add(self)

#     async def onLeave(self, details):
#         self.service.sessions.discard(self)


# class WsPerfMasterUiFactory2:
#     def __init__(self, slaveFactory):
#         self.slaveFactory = slaveFactory
#         self.sessions = set()

#     def slaveConnected(self, id, host, port, version, num_workers, ident):
#         self.sessions.publish(
#             "com.example.slaveConnected",
#             {
#                 "id": id,
#                 "host": host,
#                 "port": port,
...
#             return
#         self.session.publish("com.example.slaveDisconnected", {"id": id})
...
# def startServer1(self, webport, debug=False):

#     # ----------------------------
#     # WAMP server for slaves
#     # ----------------------------
#     runner1 = ApplicationRunner("ws://localhost:9090")

#     runner1.run(WsPerfMasterSession, start_reactor=False)

#     # ----------------------------
#     # Static web UI
#     # ----------------------------
#     if webport:
#         webdir_path = resources.files("autobahntestsuite") / "web/wsperfmaster"

#         with resources.as_file(webdir_path) as webdir:
#             site = Site(File(str(webdir)))
#             reactor.listenTCP(webport, site)

#     # ----------------------------
#     # WAMP server for UI
#     # ----------------------------
#     runner2 = ApplicationRunner("ws://localhost:9091")

#     runner2.run(WsPerfMasterUiSession, start_reactor=False)
