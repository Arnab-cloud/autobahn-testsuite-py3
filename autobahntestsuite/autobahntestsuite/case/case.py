...
class Case:
    ...
    def __init__(self, protocol):
        ...
        self.perMessageDeflateAccept = (
            lambda connectionRequest, acceptNoContextTakeover, acceptMaxWindowBits, requestNoContextTakeover, requestMaxWindowBits: (
                None
            )
        )
    ...
    def onConnectionLost(self, failedByMe):
        ...
        if self.p.connectionWasOpen:
            ...
            if self.expectedClose["closedByMe"] != self.p.closedByMe:
                ...
                self.resultClose = (
                    "The close code should have been %s or empty"
                    % ",".join(map(str, self.expectedClose["closeCode"]))
                )
            elif not self.p.factory.isServer and self.p.droppedByMe:
                self.behaviorClose = Case.FAILED_BY_CLIENT
                self.resultClose = (
                    "It is preferred that the server close the TCP connection"
                )
            else:
            ...
            if (
                closedByWrongEndpointIsFatal
                and self.expectedClose["closedByMe"] != self.p.closedByMe
            ):
                self.behavior = Case.FAILED
        else:
            self.behaviorClose = Case.FAILED
