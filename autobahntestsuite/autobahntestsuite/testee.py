...
class TesteeServerFactory(WebSocketServerFactory):
    ...
    def __init__(self, url, debug=False, ident=None):
        ...
        WebSocketServerFactory.__init__(
            self, url, debug=debug, debugCodePaths=debug, server=server
        )
...
class TesteeClientProtocol(WebSocketClientProtocol):
    def onOpen(self):
        if self.factory.endCaseId is None:
            ...
            print(
                "Running test case %d/%d as user agent %s on peer %s"
                % (
                    self.factory.currentCaseId,
                    self.factory.endCaseId,
                    self.factory.agent,
                    self.peer,
                )
            )
...
class TesteeClientFactory(WebSocketClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        if self.currentCaseId <= self.endCaseId:
            self.resource = "/runCase?case=%d&agent=%s" % (
                self.currentCaseId,
                self.agent,
            )
            connector.connect()
...
def startServer(wsuri, webport=None, sslKey=None, sslCert=None, debug=False):
    ...
    if webport:
        webdir = File(
            pkg_resources.resource_filename("autobahntestsuite", "web/echoserver")
        )
        web = Site(webdir)
