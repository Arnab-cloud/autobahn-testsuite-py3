...
def onMessage(self, msg, binary):
    if binary != self.BINARY or len(msg) != self.LEN:
        self.behavior = Case.FAILED
        self.result = (
            "Echo'ed message type or length differs from what I sent (got binary = %s, payload length = %s)."
            % (binary, len(msg))
        )
...
for b in [False, True]:
    ...
    for s in tests:
        if b:
            ...
            cc = "Case9_7_%d"
        DESCRIPTION = (
            """Send %d %s messages of payload size %d to measure implementation/network RTT (round trip time) / latency."""
            % (s[1], mt, s[0])
        )
        EXPECTATION = (
            """Receive echo'ed %s messages (with payload as sent). Timeout case after %d secs."""
            % (mt, s[2])
        )
        C = type(
            cc % i,
