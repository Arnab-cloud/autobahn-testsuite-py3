...
class TestDb:
    ...
    def newRun(self, specId):
        ...
        def do(txn):
            txn.execute(
                "SELECT mode FROM testspec WHERE id = ? AND valid_to IS NULL", [specId]
            )
            res = txn.fetchone()
            if res is None:
    ...
    def generateCasesByTestee(self, specId):
        ...
        def do(txn):
            ...
            txn.execute(
                "SELECT valid_to, mode, caseset, spec FROM testspec WHERE id = ?",
                [specId],
            )
            row = txn.fetchone()
    ...
    def _checkTestSpec(self, spec):
        ...
        if spec["caseset"] not in sig_spec_casesets:
            raise Exception(
                "invalid caseset '%s' in test specification" % spec["caseset"]
            )
        ...
        def do(txn):
            ...
            if res is not None:
                ...
                if currSpec == data:
                    ...
                    op = "U"
                    txn.execute(
                        "UPDATE testspec SET valid_to = ? WHERE id = ?", [now, currId]
                    )
            else:
                beforeId = None
                op = "I"
    ...
    def getSpecByName(self, specName):  # Status: OK.
        ...
        def do(txn):
            ...
            if res is None:
                raise Exception(
                    "no (active) test specification with name '%s'" % specName
                )
            else:
                id, data = res
    ...
    def getTestResult(self, resultId):
        ...
        def do(txn):
            txn.execute(
                "SELECT id, testrun_id, testee, c1, c2, c3, c4, c5, passed, duration, result FROM testresult WHERE id = ?",
                [resultId],
            )
            res = txn.fetchone()
            if res is None:
            ...
            result.id, result.runId, result.testeeName, result.caseName = (
                id,
                runId,
                testeeName,
                caseName,
            ...
            result.log = []
            txn.execute(
                "SELECT timestamp, sessionidx, sessionid, line FROM testlog WHERE testresult_id = ? ORDER BY lineno ASC",
                [result.id],
            )
    ...
    def getTestRunSummary(self, runId):
        ...
        def do(txn):
            ...
            txn.execute(
                "SELECT testee, SUM(passed), COUNT(*) FROM testresult WHERE testrun_id = ? GROUP BY testee",
                [runId],
            )
            res = txn.fetchall()
            r = {}
