...
PEERSET1 = [[TOPIC_PUBLISHED_TO], [TOPIC_PUBLISHED_TO]]
...
SETTINGS1 = [
    ...
    (
        ...
        None,
        [0, 1],
    ),  # exclude has precedence over excludeMe !
...
class WampCase2_2_x_x_Protocol(WampCraClientProtocol):
    def onSessionOpen(self):
        self.test.result.log.append(
            (
                perf_counter(),
                self.factory.peerIndex,
                self.session_id,
                "WAMP session opened to <strong>%s</strong> at <strong>%s</strong>."
                % (self.session_server, self.peer),
            )
        )
    ...
    def sendMessage(self, payload):
        self.test.result.log.append(
            (
                ...
                self.factory.peerIndex,
                self.session_id,
                '<pre class="wamp">TX => %s</pre>' % payload,
            )
        )
    ...
    def onMessage(self, payload, binary):
        self.test.result.log.append(
            (
                ...
                self.factory.peerIndex,
                self.session_id,
                '<pre class="wamp">RX <= %s</pre>' % payload,
            )
        )
    ...
    def onAuthSuccess(self, permissions):
        self.test.result.log.append(
            (
                ...
                self.factory.peerIndex,
                self.session_id,
                "WAMP session %s authenticated with credentials: <pre>%s</pre>"
                % (self.session_id, self.test.testee.auth),
            )
        )
        self.main()
    ...
    def onAuthError(self, e):
        ...
        self.test.result.log.append(
            (
                ...
                self.factory.peerIndex,
                self.session_id,
                "WAMP authentication error: %s" % details,
            )
        )
...
class WampCase2_2_x_x_Factory(WampClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        if self.proto and hasattr(self.proto, "session_id"):
            ...
            sid = None
        self.test.result.log.append(
            (perf_counter(), self.peerIndex, sid, "Client connection lost: %s" % reason)
        )
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        self.test.result.log.append(
            (
                ...
                self.peerIndex,
                None,
                "Client connection failed: %s" % reason,
            )
        )
        ...
        def test():
            ...
            for pl in payloads:
                if self.params.publicationMethod == 0:
                    ...
                    ENDPOINT = (
                        "http://api.testsuite.wamp-proto.org/testee/control#dispatch"
                    )
                    ...
                    raise Exception(
                        "no such publication method: %s" % self.params.publicationMethod
                    )
                ...
                if self.params.publicationMethod == 0:
                    msg = (
                        "Published event to topic <pre>%s</pre> %sand payload <pre>%s</pre>"
                        % (topic, s_args, pl)
                    )
                elif self.params.publicationMethod == 1:
                    msg = (
                        "Initiated server dispatched event to topic <pre>%s</pre> %sand payload <pre>%s</pre>"
                        % (topic, s_args, pl)
                    )
                else:
                    msg = ""
            ...
            def beforewait():
                self.result.log.append(
                    (
                        ...
                        None,
                        None,
                        "Sleeping for <strong>%s ms</strong> ..." % (1000.0 * wait),
                    )
                )
            ...
            if self.params.publicationMethod == 1 and len(d_pl) > 0:
                ...
                def onres(res):
                    self.result.log.append(
                        (perf_counter(), None, None, "Event init call result: %s" % res)
                    )
                    beforewait()
        ...
        def done(res):
            ...
            if len(clientErrors) > 0:
                ...
                if False and not passed:
                    ...
                    print("OBSERVED")
                    print(self.result.observed, end="\n\n")
...
def generate_WampCase2_2_x_x_classes(
    baseIndex, settings, payloads, publicationMethod=0
):
...
the sessions %s""" % ", ".join(
                ["<strong>%s</strong>" % x for x in params.expectedReceivers]
            )
