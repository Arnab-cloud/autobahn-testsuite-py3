...
class CaseSet:
    def __init__(
        self, CaseSetName, CaseBaseName, Cases, CaseCategories, CaseSubCategories
    ):
    ...
    def parseSpecCases(self, spec):
        ...
        if spec.get("exclude-cases", None):
            excludeCases = self.resolveCasePatternList(spec["exclude-cases"])
        else:
            excludeCases = []
        c = list(set(specCases) - set(excludeCases))
        cases = [
            self.caseIdTupletoId(y)
            for y in sorted([self.caseIdtoIdTuple(x) for x in c])
        ]
        return cases
    ...
    def parseExcludeAgentCases(self, spec):
        ...
        """
        if spec.get("exclude-agent-cases", None):
