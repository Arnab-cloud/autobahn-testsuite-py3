...
def createUtf8TestSequences() -> list[tuple[str, list[tuple[bool, bytes]]]]:
    ...
    UTF8_TEST_SEQUENCES: list[tuple[str, list[tuple[bool, bytes]]]] = []

    # 1 Some correct UTF-8 text
    vss = b"\xce\xba\xe1\xbd\xb9\xcf\x83\xce\xbc\xce\xb5"
    vs: tuple[str, list[tuple[bool, bytes]]] = ("Some valid UTF-8 sequences", [])
    ...
    vs[1].append((False, bytes(range(0x80, 0xBF))))
    UTF8_TEST_SEQUENCES.append(vs)
    ...
    def genBytes(up: int, low: int):
        for i in range(up, low):
            yield i
            yield 0x20

    vs[1].extend((False, bytes(genBytes(mm[0], mm[1]))) for mm in m)
    UTF8_TEST_SEQUENCES.append(vs)
    ...
    vs[1].extend((False, kk) for kk in k)
    UTF8_TEST_SEQUENCES.append(vs)
...
def test_utf8_incremental(validator, withPositions=True):
    ...
    if withPositions:
        k = 4
        print(
            "testing validator %s on incremental detection with positions" % validator
        )
    else:
        k = 2
        print(
    ...
    if self.isValid:
        self.expected[Case.OK] = [
            (
                ...
                else self.PAYLOAD.decode("utf-8", errors="replace"),
                False,
            )
        ]
        self.expectedClose = {
    ...
    for p in t[1]:
        ...
        C = type(
            ...
            (Case,),
            {
                "PAYLOAD": p[1],
                "isValid": p[0],
                "DESCRIPTION": f"{desc}<br><br>Payload: 0x{binascii.b2a_hex(p[1].encode() if isinstance(p[1], str) else p[1])}",
                "EXPECTATION": f"{exp}",
                "__init__": __init__,
                "onOpen": onOpen,
            },
