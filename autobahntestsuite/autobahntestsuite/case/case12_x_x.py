...
def init(self):
    ...
    with resources.as_file(
        resources.files("autobahntestsuite") / f"testdata/{TEST_DATA['file']}"
    ) as file:
        fn = str(file)

    # fn = pkg_resources.resource_filename(
    #     "autobahntestsuite", "testdata/%s" % self.TESTDATA["file"]
    # )
...
def onMessage(self, msg, binary):
    ...
    if (
        binary != self.TESTDATA["binary"]
        or len(msg) != self.LEN
        or received_hash != self._expected_hash
    ):
        self.behavior = Case.FAILED
        ...
        if binary != self.TESTDATA["binary"]:
            self.result = "Echo'ed message type differs from what I sent (got binary {0}, expected binary {1}).".format(
                binary, self.TESTDATA["binary"]
            )

        elif len(msg) != self.LEN:
            self.result = "Echo'ed message length differs from what I sent (got length {0}, expected length {1}).".format(
                len(msg), self.LEN
            )

        elif received_hash != self._expected_hash:
            self.result = "Echo'ed message contents differs from what I sent (got SHA1 {0}, expected SHA1 {1}).".format(
                binascii.hexlify(received_hash), binascii.hexlify(self._expected_hash)
            )
...
for td in WS_COMPRESSION_TESTDATA_KEYS:
    ...
    fn = ""
    with resources.as_file(
        resources.files("autobahntestsuite")
        / f"testdata/{WS_COMPRESSION_TESTDATA[td]['file']}"
    ) as file:
        fn = str(file)

    # fn = pkg_resources.resource_filename(
    #     "autobahntestsuite", "testdata/%s" % WS_COMPRESSION_TESTDATA[td]["file"]
    # )
    fileSize = os.path.getsize(fn)

    Case12_X_X_CaseSubCategories["12.%d" % j] = WS_COMPRESSION_TESTDATA[td]["desc"] + (
        " (%s, %s bytes)" % ("binary" if isBinary else "utf8", fileSize)
    )
    ...
    for s in MSG_SIZES:
        cc = "Case12_%d_%d" % (j, i)
        DESCRIPTION = (
            """Send %d compressed messages each of payload size %d, auto-fragment to %s octets. Use default permessage-deflate offer."""
            % (s[1], s[0], s[3])
        )
        EXPECTATION = (
            """Receive echo'ed messages (with payload as sent). Timeout case after %d secs."""
            % (s[2])
        )
        C = type(
            cc,
...
def accept3(self, offers):
    """
    server accept (request_no_context_takeover, request_max_window_bits): [(False, 9)]
    """
    for offer in offers:
        if isinstance(offer, PerMessageDeflateOffer):
...
def accept5(self, offers):
    """
    server accept (request_no_context_takeover, request_max_window_bits): [(True, 9)]
    """
    for offer in offers:
        if isinstance(offer, PerMessageDeflateOffer):
            if offer.acceptNoContextTakeover and offer.acceptMaxWindowBits:
                return PerMessageDeflateOfferAccept(
                    offer, request_max_window_bits=9, request_no_context_takeover=True
                )
...
def accept6(self, offers):
    ...
    for offer in offers:
        if isinstance(offer, PerMessageDeflateOffer):
            if offer.acceptNoContextTakeover and offer.acceptMaxWindowBits:
                return PerMessageDeflateOfferAccept(
                    offer, request_max_window_bits=15, request_no_context_takeover=True
                )


def accept7(self, offers):
    """
    server accept (request_no_context_takeover, request_max_window_bits): [(True, 9), (True, 0), (False, 0)]
    """
    ...
    (
        ...
        [
            PerMessageDeflateOffer(
                request_no_context_takeover=True, request_max_window_bits=0
            )
        ],
    ),
    (
        ...
        [
            PerMessageDeflateOffer(
                request_no_context_takeover=False, request_max_window_bits=9
            )
        ],
    ),
    (
        ...
        [
            PerMessageDeflateOffer(
                request_no_context_takeover=False, request_max_window_bits=15
            )
        ],
    ),
    (
        ...
        [
            PerMessageDeflateOffer(
                request_no_context_takeover=True, request_max_window_bits=9
            )
        ],
    ),
    (
        ...
        [
            PerMessageDeflateOffer(
                request_no_context_takeover=True, request_max_window_bits=15
            )
        ],
    ),
    (
        ...
        [
            ...
            PerMessageDeflateOffer(request_no_context_takeover=True),
            PerMessageDeflateOffer(),
        ],
    ),
]
...
for dp in DEFLATE_PARAMS:
    ...
    with resources.as_file(
        ...
        fn = str(file)
    # fn = pkg_resources.resource_filename(
    #     "autobahntestsuite", "testdata/%s" % TEST_DATA["file"]
    # )
    fileSize = os.path.getsize(fn)

    co_desc = "client offers (request_no_context_takeover, request_max_window_bits): {0}".format(
        [(x.request_no_context_takeover, x.request_max_window_bits) for x in co]
    )
    sa_desc = sa.__doc__.strip()

    Case13_X_X_CaseSubCategories["13.%d" % j] = (
        TEST_DATA["desc"]
        + (" (%s, %s bytes)" % ("binary" if isBinary else "utf8", fileSize))
        + " - "
        + co_desc
    ...
    for s in MSG_SIZES:
        cc = "Case13_%d_%d" % (j, i)
        DESCRIPTION = (
            """Send %d compressed messages each of payload size %d, auto-fragment to %s octets. Use permessage-deflate %s"""
            % (s[1], s[0], s[3], co_desc)
        )
        EXPECTATION = (
            """Receive echo'ed messages (with payload as sent). Timeout case after %d secs."""
