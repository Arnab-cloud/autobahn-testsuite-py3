...
class NumberService(object):
    ...
    def add(self, *numbers):
        ...
        assert [
            n
            for n in numbers
            if type(n) not in [types.IntType, types.FloatType, types.LongType]
        ] == []
        return sum(numbers)
...
class TesteeWampServerProtocol(wamp.WampServerProtocol):
    ...
    def initializeServices(self):
        ...
        self.registerMethodForRpc(
            ...
            self,
            TesteeWampServerProtocol.testDispatch,
        )
        ...
        for case_id in [ECHO_NUMBER_ID, ECHO_STRING_ID]:
            for idx in range(1, 5):
                self.registerMethodForRpc(
                    setupUri(case_id, idx), self.echo_service, EchoService.echo
                )
        self.registerMethodForRpc(
            setupUri(ECHO_DATE_ID), self.echo_service, EchoService.echo
        )
        self.registerMethodForRpc(
            setupUri(CONCAT_STRINGS_ID), self.string_service, StringService.concat
        )
        for case_id in [ADD_TWO_NUMBERS_ID, ADD_THREE_NUMBERS_ID]:
            self.registerMethodForRpc(
                setupUri(case_id), self.number_service, NumberService.add
            )
